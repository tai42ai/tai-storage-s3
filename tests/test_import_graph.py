...
``tai_storage_s3`` is on the allowlist. The rule (see the README): the shipped
package imports
the AWS SDK stack (aioboto3 / aiobotocore / boto3 / botocore), the shared
platform substrate (tai-contract + tai-kit, pydantic, pydantic-settings)
...
The runtime walk imports ``tai_storage_s3`` and every submodule in a fresh
subprocess, then inspects ``sys.modules``. Running it in a subprocess that
imports ONLY ``tai_storage_s3`` means the assertion covers the SHIPPED package's
...
PACKAGE = "tai_storage_s3"
ALLOWED_FIRST_PARTY = frozenset({PACKAGE, "tai_contract", "tai_kit"})

# Every third-party root the shipped ``tai_storage_s3`` graph pulls in -- the
# declared runtime dependencies plus their resolved closure. Compiled extension
# modules appear under the bare top-level name they register (``_cffi_backend`` is the C
# extension of the ``cffi`` distribution), so they are listed alongside their
# providing distribution. Adding a runtime dependency that brings a new root means adding
...
def test_shipped_package_imports_only_allowlisted_roots() -> None:
    ...
    assert result.returncode == 0, (
        f"importing the shipped tai_storage_s3 graph failed:\nstdout:\n{result.stdout}\nstderr:\n{result.stderr}"
    )

    offenders = [line for line in result.stdout.splitlines() if line]
    assert offenders == [], f"non-allowlisted roots in the tai_storage_s3 module graph: {offenders}"
...
def test_shipped_sources_name_only_allowlisted_roots() -> None:
    ...
    assert offenders == {}, f"non-allowlisted import roots in the tai_storage_s3 sources: {offenders}"
